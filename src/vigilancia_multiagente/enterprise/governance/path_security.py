...
Adapted from Hermes Agent — Original file: ``tools/path_security.py``
License: MIT (see ``documentation/hermes agent/hermes-agent/LICENSE``).

Spec 021 FR-025/026: governance F1 from Hermes, port verbatim (≤400 LOC,
attribution header, smoke-tested per module).

Extracts the ``resolve() + relative_to()`` and ``..`` traversal check
patterns previously duplicated across skill_manager_tool, skills_tool,
skills_hub, cronjob_tools, and credential_files in Hermes upstream.
"""
