...
Adapted from Hermes Agent — Original file: ``tools/url_safety.py``
License: MIT (see ``documentation/hermes agent/hermes-agent/LICENSE``).

Spec 021 FR-025: F1 governance. Prevents SSRF (Server-Side Request Forgery)
where a malicious prompt or skill could trick the agent into fetching
internal resources like cloud metadata endpoints (169.254.169.254),
localhost services, or private network hosts.

The check can be globally disabled via ``VT_ALLOW_PRIVATE_URLS=true`` for
environments where DNS resolves external domains to private/benchmark-range
IPs. Even when disabled, cloud metadata hostnames (``metadata.google.internal``,
``169.254.169.254``) are **always** blocked — those are never legitimate
agent targets.

**Deviations from upstream**:

* ``is_truthy_value`` (Hermes ``utils.py``) is inlined as ``_is_truthy``.
* The ``hermes_cli.config.read_raw_config`` config-file path is dropped —
  the toggle now reads only from ``VT_ALLOW_PRIVATE_URLS``. A future task
  may surface this as a VT setting.
...
# Inlined from Hermes ``utils.is_truthy_value`` (constitucion #2 KISS:
# the helper is 6 LOC; pulling in the whole utils module is overkill).
...
# or any config toggle. Cloud metadata endpoints — primary SSRF target.
...
# Always-blocked IPs (cloud metadata) and link-local range. IPv4-mapped
# IPv6 variants are included because DNS resolvers may return
# ``::ffff:x.x.x.x`` for IPv4-only hosts.
...
# Exact HTTPS hostnames allowed to resolve to private/benchmark IPs.
...
def _global_allow_private_urls() -> bool:
    ...
    Reads ``VT_ALLOW_PRIVATE_URLS``. Result is cached for the process lifetime.
    """
    ...
    _cached_allow_private = _is_truthy(os.getenv("VT_ALLOW_PRIVATE_URLS"))
    return _cached_allow_private
...
def is_always_blocked_url(url: str) -> bool:
    ...
    Security floor — cloud metadata IPs / hostnames that have no legitimate
    agent use regardless of toggles. Returns False on parse errors so the
    caller decides fail-open vs fail-closed; ``is_safe_url`` is the strict
    full check.
    """
    try:
        parsed = urlparse(url)
...
def is_safe_url(url: str) -> bool:
    ...
    **Fails closed**: DNS errors and unexpected exceptions block the request.
    ...
    When ``VT_ALLOW_PRIVATE_URLS=true``, private-IP blocking is skipped.
    ...
    try:
        ...
        if allow_all_private:
            logger.debug(
                "Allowing private/internal resolution (VT_ALLOW_PRIVATE_URLS=true): %s",
                hostname,
            )
