...
Adapted from Hermes Agent — Original file: ``tools/website_policy.py``
License: MIT (see ``documentation/hermes agent/hermes-agent/LICENSE``).

Spec 021 FR-025: F1 governance. Loads a user-managed website blocklist from
``~/.vigilador/config.yaml`` (formerly ``~/.hermes/config.yaml``) and optional
shared list files. Lightweight so web/browser tools can enforce URL policy
without the full CLI config stack.
...
**Deviation from upstream**: ``hermes_constants.get_hermes_home()`` is replaced
with ``Path("~/.vigilador").expanduser()`` so the module is self-contained.
"""
...
def _vigilador_home() -> Path:
    """Replacement for Hermes ``get_hermes_home()`` — uses ``~/.vigilador``."""
    return Path("~/.vigilador").expanduser()


def _get_default_config_path() -> Path:
    return _vigilador_home() / "config.yaml"
...
def _iter_blocklist_file_rules(path: Path) -> list[str]:
    ...
    Constitucion #4: error transformation with context (warning + skip),
    not silent swallow.
    """
    try:
...
def load_website_blocklist(config_path: Path | None = None) -> dict[str, Any]:
    ...
    config.yaml on every URL check. Pass an explicit ``config_path`` to
    ...
    for shared_file in raw_shared_files:
        ...
        if not path.is_absolute():
            path = (_vigilador_home() / path).resolve()
...
def check_website_access(
    ...
    Documented fail-open semantics for config errors when no explicit
    ``config_path`` is given: a config typo should not break all web tools.
    Tests pass ``config_path`` explicitly to get strict error propagation.
    """
    if config_path is None:
        with _cache_lock:
        ...
        if _match_host_against_rule(host, pattern):
            ...
            return {
                ...
                "message": (
                    ...
                    f"'{pattern}' from {rule.get('source', 'config')}"
                ),
            }
    return None
