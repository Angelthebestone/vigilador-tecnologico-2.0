...
Adapted from Hermes Agent — Original file: ``tools/schema_sanitizer.py``
License: MIT (see ``documentation/hermes agent/hermes-agent/LICENSE``).

Spec 021 FR-025/026: governance F1 / tooling base.

**Cohesion justification (constitucion #2 KISS / AHA, doc 06 §1.1)**: this
module is **445 LOC** — borderline against the ≤400 LOC preferencia. The
file is preserved as a single module because every function shares the
same domain (JSON Schema sanitization for tool calls) and operates on the
same tree-walk primitive (``_sanitize_node``). Splitting into "sanitize" /
"strip-on-recovery" / "strip-slash-enum" submodules would force callers to
import three names instead of one for a single conceptual operation, and
would break the deep co-evolution of these helpers — they all encode
empirically-discovered backend incompatibilities. The "preferencia" yields
to single-concern cohesion here.
...
The failure modes seen in the wild and handled here:
...
in-place on a deep copy. Intentionally conservative: only modifies shapes
...
def _sanitize_single_tool(tool: dict) -> dict:
    ...
    # argument coercion can still map a model-emitted ``"null"`` string to
    # Python ``None``.
...
def _strip_top_level_combinators(params: dict, *, path: str = "<tool>") -> dict:
    ...
    Removing them at the top level discards the hint but does not change
    which argument *values* are valid — the tool handler always re-validates
    required fields. Only the *top* level is stripped; combinators nested
    inside a property's schema are preserved.
    """
    if not isinstance(params, dict):
        return params
    out = dict(params)
...
def _sanitize_node(node: Any, path: str) -> Any:
    ...
    - Normalizes ``type: [X, "null"]`` arrays to single ``type: X``
      (keeping ``nullable: true`` as a hint).
...
def strip_pattern_and_format(tools: list[dict]) -> tuple[list[dict], int]:
    ...
    *Reactive* sanitizer invoked only when llama.cpp's
    ``json-schema-to-grammar`` converter has rejected a tool schema with
    HTTP 400. llama.cpp's regex engine supports only a small subset of
    ECMAScript regex — it rejects escape classes like ``\\d``, ``\\w``,
    ``\\s`` and most ``format`` values. Cloud providers accept these
    keywords fine and rely on them as prompting hints, so we keep them
    in the default schema and only strip on demand.

    The strip operates on a sibling of ``type`` (so schema keywords are
    removed) — a property literally *named* ``pattern`` is not affected
    because property names live in the ``properties`` dict, not as siblings
    of ``type``.
    """
    if not tools:
        return tools, 0
...
def strip_slash_enum(tools: list[dict]) -> tuple[list[dict], int]:
    ...
    (the request fails with HTTP 400 before any token is emitted). Most
    commonly hit by MCP-derived tools whose enum lists HuggingFace model
    IDs (``Qwen/Qwen3.5-0.8B``) or owner/name environment IDs. The
    constraint is purely a prompting hint; dropping it lets the model see
    the field description and pick a value, without xAI tripping on the slash.
    """
    if not tools:
        return tools, 0
