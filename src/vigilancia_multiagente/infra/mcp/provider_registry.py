...
class MCPProviderRegistry:
    ...
    def ensure_standard_providers(self, settings: Settings) -> None:
        ...
        defaults = {
            ...
            "sandbox": MCPProviderConfig(
                name="sandbox",
                transport=MCPTransport.STDIO,
                base_url_or_command="python",
                arguments=["-m", "vigilancia_multiagente.infra.mcp.sandbox.server"],
                auth_mode=MCPAuthMode.NONE,
                timeout_ms=120000,
                retry_policy=RetryPolicy(max_attempts=1, backoff_ms=500),
                enabled_tools=("execute_code", "list_libraries", "visualize"),
                capabilities=("code_execution", "analytics", "visualization"),
            ),
            "markitdown": MCPProviderConfig(
                name="markitdown",
                transport=MCPTransport.STDIO,
                base_url_or_command="markitdown-mcp",
                arguments=[],
                auth_mode=MCPAuthMode.NONE,
                timeout_ms=60000,
                retry_policy=RetryPolicy(max_attempts=1, backoff_ms=500),
            ...
            "openalex": MCPProviderConfig(
                name="openalex",
                transport=MCPTransport.STDIO,
                base_url_or_command="npx",
                arguments=["-y", "openalex-research-mcp"],
                auth_mode=MCPAuthMode.NONE,
                timeout_ms=settings.mcp_default_timeout_ms,
                retry_policy=RetryPolicy(
                    max_attempts=settings.mcp_default_retry_limit, backoff_ms=500
                ...
                environment={
                    **_env("OPENALEX_EMAIL", settings.openalex_email),
                    **_env("OPENALEX_API_KEY", _secret(settings.openalex_api_key)),
                },
            ),
