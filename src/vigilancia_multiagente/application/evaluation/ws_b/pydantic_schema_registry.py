...
class PydanticExtractionSchemaRegistry:
    ...
    def register(self, source_type: str, domain: str, schema: type[BaseModel]) -> None:
        self._models[(source_type, domain)] = schema

    def register_from_extraction_schema(self, extraction_schema: ExtractionSchema) -> None:
    ...
    def get_schema(self, source_type: str, domain: str) -> ExtractionSchema:
        ...
        if key not in self._models:
            raise KeyError(
                f"No schema registered for source_type={source_type!r} domain={domain!r}"
            )
    ...
    def validate(self, raw: dict[str, object], schema: ExtractionSchema) -> dict[str, object]:
...
def _build_model_from_json_schema(schema_dict: dict[str, object]) -> type[BaseModel]:
...
def _dynamic_model_from_dict(schema: dict[str, object]) -> type[BaseModel]:
    fields: dict[str, tuple[type, Any]] = {}
