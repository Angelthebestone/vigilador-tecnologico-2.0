...
class ReportGenerator:
    ...
    async def _build_technical_report(self, findings: dict) -> dict:
        ...
        sections.append(self._section("Methodology",
        ...
        source_text = "\n".join(f"- {s.get('name', s)} (confidence: {s.get('score', 'N/A')})" for s in sources[:20])
        sections.append(self._section("Data Sources", source_text or "No sources recorded."))

        analytics = findings.get('analytics', {})
        bio_text = f"Novelty scores computed for {len(analytics.get('novelty_scores', []))} papers\n"
    ...
    async def _build_executive_brief(self, findings: dict) -> dict:
        ...
        rec_text = "\n".join(f"- {r}" for r in recs[:10]) or "Generate recommendations from branch findings."
        ...
        comp_text += f"Market positioning: {comp.get('overview', 'Analysis available in findings.')}"
    ...
    async def _build_risk_report(self, findings: dict) -> dict:
        ...
        reg_text = "\n".join(f"- {r.get('title', r)} [Source: {r.get('source', 'N/A')}]" for r in regs[:20])
        ...
        comp_text = "\n".join(f"- {c}" for c in compliance[:10]) or "Review branch findings for compliance-related content."
