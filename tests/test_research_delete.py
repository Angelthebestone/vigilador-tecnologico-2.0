...
def _seeded_repo(memory_repositories):
    ...
    asyncio.get_event_loop().run_until_complete(repo.create(session))
    return session, repo
...
class TestDeleteSession:
    def test_delete_removes_session(self, _seeded_repo):
        ...
        assert asyncio.get_event_loop().run_until_complete(repo.get_by_id(session.id)) is not None
        asyncio.get_event_loop().run_until_complete(repo.delete(session.id))
        assert asyncio.get_event_loop().run_until_complete(repo.get_by_id(session.id)) is None

    def test_delete_nonexistent_is_noop(self, memory_repositories):
        import asyncio

        repo = memory_repositories["session_repo"]
        asyncio.get_event_loop().run_until_complete(repo.delete(uuid4()))
    ...
    def test_get_returns_none_after_delete(self, _seeded_repo):
        ...
        session, repo = _seeded_repo
        asyncio.get_event_loop().run_until_complete(repo.delete(session.id))
        result = asyncio.get_event_loop().run_until_complete(repo.get_by_id(session.id))
