...
def test_lru_evict(cache: EmbeddingCache) -> None:
    ...
    small_cache = EmbeddingCache(cache_dir=cache._cache_dir, filename="small.json", max_memory_entries=2)
    ...
    assert "content1" not in [k for k in small_cache._l1_cache.keys()] # Keys are hashed, but we can check length
...
def test_flush_atomic_write(cache: EmbeddingCache, cache_dir: Path) -> None:
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        data = json.load(f)
        assert "content1" in data or any(k for k in data.keys())  # Key is hashed
