...
async def test_sandbox_execute_code():
    ...
    result = await _call_sandbox("execute_code", {"code": "print('hello world')", "timeout": 10})
...
async def test_sandbox_timeout():
    ...
    result = await _call_sandbox(
        "execute_code", {"code": "import time; time.sleep(10)", "timeout": 2}
    )
    assert result is not None
