...
def client(monkeypatch):
    ...
    monkeypatch.setattr(
        "vigilancia_multiagente.infra.llm.minimax_client.get_settings", lambda: FakeSettings()
    )
    c = MiniMaxClient()
    c._client = AsyncMock()
    return c
...
def test_complete_sends_max_tokens(client: MiniMaxClient):
    ...
    client._client.post = AsyncMock(
        return_value=_ok_response([{"message": {"content": "hello", "tool_calls": []}}])
    )
...
def test_complete_sends_stream_true(client: MiniMaxClient):
    ...
    asyncio.run(
        client.complete(messages=[MiniMaxMessage(role="user", content="test")], stream=True)
    )
...
def test_complete_raises_without_api_key(monkeypatch):
    ...
    monkeypatch.setattr(
        "vigilancia_multiagente.infra.llm.minimax_client.get_settings",
        lambda: FakeSettings(minimax_api_key=None),
...
def test_parse_response_handles_missing_reasoning():
    ...
    payload = {"choices": [{"message": {"content": "Just answer", "tool_calls": []}}]}
    result = _parse_response(payload)
