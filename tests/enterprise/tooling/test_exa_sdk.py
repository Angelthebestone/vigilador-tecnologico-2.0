...
def test_exa_unknown_tool_name():
    ...
    tool = ExaTool()
    with (
        patch.dict("os.environ", {"VT_EXA_API_KEY": "test-key"}),
        pytest.raises(ValueError, match="unknown tool_name"),
    ):
        import asyncio
...
def test_exa_search_uses_sdk():
    ...
    with (
        patch.dict("os.environ", {"VT_EXA_API_KEY": "test-key"}),
        patch("vigilancia_multiagente.enterprise.tooling.builtin.research.exa.Exa") as MockExa,
    ):
        ...
        result = asyncio.run(
            tool.execute("semantic_search", {"query": "AI safety", "num_results": 5})
        )
